...
class ArbMonitor:
    ...
    def _fetch_metadata(self):
        ...
        usdc_addr = cfg("tokens", "USDC", "address").lower()
        ...
        for p in new_pools:
            ...
            calls.append({"target": u_addr, "callData": u_pool.encodeABI("token0")})
            calls.append({"target": u_addr, "callData": u_pool.encodeABI("token1")})
            # Camelot metadata
            c_pool = self.w3.eth.contract(address=c_addr, abi=ERC20_ABI)
            calls.append({"target": c_addr, "callData": c_pool.encodeABI("token0")})
            calls.append({"target": c_addr, "callData": c_pool.encodeABI("token1")})
        ...
        try:
            ...
            for rd in return_data:
                token_addrs.add(checksum(self.w3.codec.decode(["address"], rd)[0]))

            dec_calls = []
            sorted_tokens = sorted(list(token_addrs))
            for t in sorted_tokens:
                erc20 = self.w3.eth.contract(address=t, abi=ERC20_ABI)
                dec_calls.append({"target": t, "callData": erc20.encodeABI("decimals")})

            _, dec_data = self.mc.functions.aggregate(dec_calls).call()
            token_decimals = {}
            for i, t in enumerate(sorted_tokens):
                token_decimals[t.lower()] = self.w3.codec.decode(["uint8"], dec_data[i])[0]

            # Store metadata
            for i, p in enumerate(new_pools):
                u_t0 = self.w3.codec.decode(["address"], return_data[i*4])[0].lower()
                u_t1 = self.w3.codec.decode(["address"], return_data[i*4+1])[0].lower()
                c_t0 = self.w3.codec.decode(["address"], return_data[i*4+2])[0].lower()
                c_t1 = self.w3.codec.decode(["address"], return_data[i*4+3])[0].lower()

                self.metadata[p["univ3Pool"]] = {
                    "token0": u_t0, "token1": u_t1,
                    "dec0": token_decimals.get(u_t0, 18), "dec1": token_decimals.get(u_t1, 18),
                    "is_token1_usdc": u_t1 == usdc_addr
                }
                self.metadata[p["camelotPool"]] = {
                    "token0": c_t0, "token1": c_t1,
                    "dec0": token_decimals.get(c_t0, 18), "dec1": token_decimals.get(c_t1, 18),
                    "is_token1_usdc": c_t1 == usdc_addr
                }
        except Exception as e:
            logger.error(f"Failed to fetch pool metadata: {e}")
